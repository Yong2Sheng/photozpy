...
class Reduction():
    ...
    def apply_bias_correction(self, save_location=""):
        ...
        if len(master_bias_path) == 0:
            ...
            raise ValueError(
                f"You have more than two master bias frames --> {master_bias_path}!")
        ...
        correct_collection = CollectionManager.filter_collection(
            self._image_collection, **{"IMTYPE": ["Dark", "Light", "Flat"]})
    ...
    def apply_dark_correction(self, save_location=""):
        ...
        if len(master_dark_path) == 0:
            ...
            raise ValueError(
                f"You have more than one master dark frame --> {master_dark_path}!")
        ...
        correct_collection = CollectionManager.filter_collection(
            self._image_collection, **{"IMTYPE": ["Light", "Flat"]})
    ...
    def apply_flat_correction(self, save_location=""):
        ...
        for filter in filters:
            ...
            master_flat_collection = CollectionManager.filter_collection(
                self._image_collection, **{"IMTYPE": ["Master Flat"], "FILTER": [filter]})
            ...
            if len(light_collection.files) == 0:
                ...
                for master_flat_ccd, flat_fname in master_flat_collection.ccds(
                    ...
                    for object_ccd, fname in light_collection.ccds(
                        ...
                        object_ccd_corrected.data = object_ccd_corrected.data.astype(
                            np.float32)  # float32 is enough precision
