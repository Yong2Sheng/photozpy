...
class Artifacts():
    ...
    @staticmethod
    def _remove_cosmic_ray(fits_path, plot_before_after=False,
                           plot_mask=False, norm_percent=99.9, save_location=None, **kwargs):
        """
        ...
        kwargs : dict
            The parameters and arguments passing to `lacosmic`.
        ...
        if plot_before_after:
            ...
            plot_image(
                ccd_data=newccd,
                norm_percent=norm_percent,
                save_location=fits_path.parent,
                fname_append=file_name)
    ...
    def remove_cosmic_ray(self, plot_before_after=False, plot_mask=False,
                          norm_percent=99.9, save_location=None, headers_values=None, **kwargs):
        """
        ...
        if headers_values is not None:
            image_collection = CollectionManager.filter_collection(
                self._image_collection, **headers_values)
        else:
