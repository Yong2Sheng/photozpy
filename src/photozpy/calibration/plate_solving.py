...
class PlateSolving:
    ...
    def add_wcs(
        ...
        std_collection = CollectionManager.filter_collection(
            ...
            **{
                "IMTYPE": "Master Light",
                "OBJECT": self._sources.standard_stars[0].source_name,
            },
        )
        std_list = std_collection.files_filtered(include_path=True)
        # target_list = [i for i in all_image_list if not i in std_list]
        target_list = [
            i for i in all_image_list if i not in std_list
        ]  # PEP 8: For negated membership tests, use not in rather than not ... in.
