...
class Combine:
    ...
    def combine_bias_or_dark(
        ...
        for object_name in object_names:  # note that bias and dark don't have filters
            ...
            if image_number == 0:
                ...
                ).stem  # get the filename stem: crab_sdss_g.fits --get--> crab_sdss_g
                ...
                new_name_path = name_path.with_name(
                    f"Master_{object_name}{fname_suffix}"
                )  # the new path
                name_path.rename(
                    new_name_path
                )  # rename the file, notice that you have to use the full path to rename, not just the file name
                ...
                if method == "sigma clip":
                    combined = ccdpro_combine(
                        image_list_to_combine,
                        method="average",
                        sigma_clip=True,
                ...
                combined.write(
                    Path(save_location) / f"Master_{object_name}.fits", overwrite=True
                )
    ...
    def combine_light_or_flat(
        ...
        object_names = HeaderManipulation.get_header_values(
            collection_to_combine, header="object"
        )  # It seems only the lower cases work
        object_names = [*set(object_names)]  # remove duplicate elements
        ...
        for object_name in object_names:
            ...
            for filter in filters:
                ...
                collection_temp = CollectionManager.filter_collection(
                    ...
                    **{"OBJECT": [object_name], "FILTER": [filter]},
                )
                image_list_to_combine = list(
                    collection_temp.files_filtered(include_path=True)
                )  # this is the list of the absolute path to the fits images to be combined
                image_filenames = list(
                ...
                if image_number == 0:
                    ...
                    HeaderManipulation.edit_headers_with_fitsio(
                        collection_temp,
                        save_location=save_location,
                    ...
                    fname_stem = Path(
                        image_list_to_combine[0]
                    ).stem  # get the filename stem: crab_sdss_g.fits --get--> crab_sdss_g, it only contains one fits file
                    fname_suffix = Path(
                        image_list_to_combine[0]
                    ...
                    if image_type == "Light":
                        ...
                        combined.meta["RDNOISE"] = rdnoise / np.sqrt(
                            image_number
                        )  # update the RDNOISE
