...
class CollectionManager():
    ...
    @staticmethod
    def plot_collection(image_collection, save_location=None,
                        norm_percent=99.9, **headers_values):
        """
        ...
        # collection_to_plot =
        # CollectionManager.refresh_collection(image_collection, rescan=True)
        # assigined but not used
        if headers_values != {}:
            image_collection = CollectionManager.filter_collection(
                image_collection, **headers_values)
        fits_path_to_plot = list(
            image_collection.files_filtered(include_path=True))
