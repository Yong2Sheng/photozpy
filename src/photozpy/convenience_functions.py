...
def get_alain_image(
    ...
    if save_image is True:
        ...
        if sky_region is None:
            ...
            ax1.set_ylim(
                pixereg.center.y - y_npix * 0.1, pixereg.center.y + y_npix * 0.1
            )
...
def convert_coords(
    image_path=None, wcs=None, skycoords=None, pixelcoords=None, verbose=False
):
    """
    ...
    if skycoords is not None and pixelcoords is None:
        ...
        pixelcoords = np.array(
            (pixelcoords)
        ).T  # transfer the array so it's ra/dec in each column
        out = pixelcoords  # output variable
        if out.ndim == 1:
            out = out.reshape(
                1, 2
            )  # if the input skycoord only has one pair of coordinates, it will return an 1D array [573.8259494 ,  72.66086389] instead of 2D array [[573.8259494 ,  72.66086389]]
        if verbose:
...
def plot_image(
    ...
    if fits_path is not None:
        ...
        if fname_append:
            image_name = (
                ccd_data.header["OBJECT"]
                + "_"
                + ccd_data.header["FILTER"]
                + fname_append
                + ".png"
            )
        else:
    ...
    if annulus_apertures is not None:
        _ann_patches = annulus_apertures.plot(
            color="red", lw=1, label="Background annulus"
        )

    # Determine the final size of image (xlim and ylim)
    if (
        skycoords is None
        and pixelcoords is None
        and circular_apertures is None
        and annulus_apertures is None
    ):
