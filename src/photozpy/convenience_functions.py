...
def get_alain_image(image_path=None, hdu=1, wcs=None, save_dir=None, sky_region=None, min_cut=0.5,
                    max_cut=99.5, logstretch=10, source_name=None, hips="CDS/P/DSS2/blue", save_image=True):
    """
    ...
    if save_image is True:
        ...
        if sky_region is None:
            ...
            ax1.set_xlim(
                pixereg.center.x - x_npix * 0.1,
                pixereg.center.x + x_npix * 0.1)
            ax1.set_ylim(
                pixereg.center.y - y_npix * 0.1,
                pixereg.center.y + y_npix * 0.1)
...
def estimate_background(image_path=None, array_data=None, hdu=0,
                        unit=None, sigma=3, box_size=(20, 20), filter_size=(5, 5), **kwargs):
    """
...
def ungz_file(file_path, out_dir=None, return_path=False, overwrite=True):
    ...
    if ungzed_path.exists():
        if overwrite is True:
            ...
            raise OSError(f"File already exists: {ungzed_path}.")
...
def convert_coords(image_path=None, wcs=None, skycoords=None,
                   pixelcoords=None, verbose=False):
    """
    ...
    if image_path is None and wcs is None:
        raise TypeError(
            "You must give a file path or asrtropy.wcs.WCS obkect as the input!")

    if skycoords is None and pixelcoords is None:
        raise TypeError(
            "You must give sky coordinates or pixel coordinates as the input!")

    elif skycoords is not None and pixelcoords is not None:
        raise TypeError(
            "Please only input the sky coordinates or the pixel coordinates!")
    ...
    if skycoords is not None and pixelcoords is None:
        pixelcoords = wcs_object.world_to_pixel(skycoords)
        # transfer the array so it's ra/dec in each column
        pixelcoords = np.array((pixelcoords)).T
        out = pixelcoords  # output variable
        if out.ndim == 1:
            # if the input skycoord only has one pair of coordinates, it will
            # return an 1D array [573.8259494 ,  72.66086389] instead of 2D
            # array [[573.8259494 ,  72.66086389]]
            out = out.reshape(1, 2)
        if verbose:
            print("Conversion from sky coordinates to pixel coordinates completed!")

    elif skycoords is None and pixelcoords is not None:
...
def plot_image(fits_path=None,
    ...
    elif ccd_data is not None:
        fits_data = ccd_data.data
        if fname_append:
            image_name = ccd_data.header["OBJECT"] + "_" + \
                ccd_data.header["FILTER"] + fname_append + ".png"
        else:
            image_name = ccd_data.header["OBJECT"] + "_" + \
                ccd_data.header["FILTER"] + fname_append + ".png"
        if save_location is None:
    ...
    if skycoords is not None:
        ...
        plt.scatter(pixelcoords_from_skycoords[:,
                                               0],
                    pixelcoords_from_skycoords[:,
                                               1],
                    marker="*",
                    s=5,
                    color="lime",
    ...
    if isinstance(pixelcoords, np.ndarray):
        ...
        plt.scatter(pixelcoords[:, 0], pixelcoords[:, 1],
                    marker="*", s=5, color="lime", label="True Coordinates")
    ...
    if circular_apertures is not None:
        centers_all = np.concatenate(
            (centers_all, circular_apertures.positions), axis=0)
        # ap_patches = circular_apertures.plot(color='orange', lw=1,
        # label='Photometry aperture')  # assigned but not used
        ...
        for row_idx in np.arange(circular_centers.shape[0]):
            ...
            # plt.annotate(f"x={x_}, y={y_}", (x_, y_))

    # if annulus_apertures is not None:
    ...
    if skycoords is None and pixelcoords is None and circular_apertures is None and annulus_apertures is None:
