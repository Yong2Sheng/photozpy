...
class MagnitudeInfo():
    ...
    def __init__(self, filters, mag_type, inst_mags=None, inst_mag_errors=None,
                 zero_points=None, cab_mags=None, cab_mag_errors=None, calibrated=False):
        """
    ...
    def _standarize_mags(mags, filters):
        ...
        if mags is None:
            ...
            if isinstance(mags[0], (Quantity, list)):

                rows = [len(i) for i in mags]

                if rows.count(rows[0]) != len(
                        rows):  # check if the number of magnitudes in different filters are equal
                    raise ValueError(
                        "The number of magnitudes in filters are not equal!")

                else:
                    if isinstance(
                            mags[0], Quantity):  # [[-99, 23]*u.mag, [-99, 56]*u.mag, [-99, 46]*u.mag, [-99, 233]*u.mag]
                        return QTable(mags,
                                      names=filters)
    ...
    def detection_significance(self, new_detection_significance):
        # for simplicity, only QTable is supported since users should not input
        # detection significance
        self._significance = QTable(new_detection_significance)
    ...
    def plot_zero_point_statistics(self, save=True):
        ...
        fig, axes = plt.subplots(
            nrows, 2, figsize=(
                12, 10), sharex=False)  # sharex=True
        ...
        for idx, filter_name in enumerate(self._filters):
            ...
            axes[int(np.floor(idx / 2)),
                 idx % 2].hlines(y=self._zero_points[filter_name].value[0] - 0.1,
                                 color='r',
                                 xmin=0,
                                 xmax=1,
