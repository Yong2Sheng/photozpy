...
class MagnitudeInfo:
    ...
    def __init__(
        ...
        self._diff_dict = (
            {}
        )  # the difference between calibrated and instrumental magnitudes, which is also the zero points, in all the filters
        self._significance = {}  # the detection significance for sources in all filters

    @staticmethod
    def _standarize_mags(mags, filters):
        """
        Standardize the magnitudes input.
        ...
        if mags is None:
            ...
            if isinstance(
                mags[0], (Quantity, list)
            ):  # [[-99, 23]*u.mag, [-99, 56]*u.mag, [-99, 46]*u.mag, [-99, 233]*u.mag] or [[-99, 23], [-99, 56], [-99, 46]]

                rows = [len(i) for i in mags]

                if rows.count(rows[0]) != len(
                    rows
                ):  # check if the number of magnitudes in different filters are equal
                    raise ValueError(
                        "The number of magnitudes in filters are not equal!"
                    )
                ...
                return QTable(mags * u.mag, names=filters)
    ...
    def inst_mag_errors(self, new_inst_mag_errors):
        self._inst_mag_errors = MagnitudeInfo._standarize_mags(
            new_inst_mag_errors, self._filters
        )
    ...
    def cab_mag_errors(self, new_cab_mag_errors):
        self._cab_mag_errors = MagnitudeInfo._standarize_mags(
            new_cab_mag_errors, self._filters
        )
    ...
    def detection_significance(self, new_detection_significance):
        self._significance = QTable(
            new_detection_significance
        )  # for simplicity, only QTable is supported since users should not input detection significance
    ...
    def mag_operator(mag_list1, mag_list2, operation):
        ...
        if len(mag_list1) == len(mag_list2):
            ...
            raise ValueError(
                "Can't broadcast magnitude quantity with more than one value!"
            )
    ...
    def calculate_zero_points(self, sigma=3, update=True):
        ...
        for filter_name in calculated_zero_points.colnames:
            ...
            zero = MagnitudeInfo.mag_operator(
                mag_list1=self._cab_mags[
                    filter_name
                ],  # .value return a list even with one number: [-18]
            ...
            zero = MagnitudeInfo.remove_outlier(
                zero.value, sigma=sigma
            )  # remove outlier and give it an unit
    ...
    def calibrate_inst_mags(self, update=True):
        ...
        for filter_name in self._inst_mags.colnames:
            ...
            calibrated = MagnitudeInfo.mag_operator(
                mag_list1=self._zero_points[
                    filter_name
                ],  # .value return a list even with one number: [-18]
                mag_list2=self._inst_mags[filter_name],
                operation="+",
    ...
    def plot_zero_point_statistics(self, save=True):
        ...
        for idx, filter_name in enumerate(self._filters):
            ...
            axes[int(np.floor(idx / 2)), idx % 2].scatter(
                x=[filter_name] * len(self._diff_dict[filter_name]),
            ...
            axes[int(np.floor(idx / 2)), idx % 2].scatter(
                ...
                label=r"$\sigma$-clipped average",
                color="lime",
            )

            axes[int(np.floor(idx / 2)), idx % 2].hlines(
                y=self._zero_points[filter_name].value[0] - 0.1,
                color="r",
                xmin=0,
                xmax=1,
                label="sigma-clipped - 0.1",
            )
            axes[int(np.floor(idx / 2)), idx % 2].hlines(
                y=self._zero_points[filter_name].value[0] + 0.1,
                color="r",
