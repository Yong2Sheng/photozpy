...
class SourceDetection:
    ...
    def find_local_peaks(
        image_path,
        std_clip_threshold=3,
        local_peak_detection_threshold=10,
        fit_box_size=41,
        plot=False,
        verbose=True,
    ):
        ...
        mean, median, std = sigma_clipped_stats(
            data, sigma=std_clip_threshold
        )  # clip the extreme values from data
        threshold = median + (
            local_peak_detection_threshold * std
        )  # calculate the threshold used to identify the peaks
        tbl = find_peaks(
            data, threshold, box_size=fit_box_size
        )  # Qtable, it's pandas-like object with columns x_peak, y_peak, and peak_value
        local_peak_positions = np.transpose(
            (tbl["x_peak"], tbl["y_peak"])
        )  # the array of pixel coordinates of the local peaks
    ...
    def find_centroids(
        ...
        for image_path in image_list:
            ...
            # detect local peaks
            local_peak_positions_ = SourceDetection.find_local_peaks(
                image_path,
            ...
            x_centroids, y_centroids = centroid_sources(
                data,
                x_local_peak_positions,
                y_local_peak_positions,
                box_size=fit_box_size,
                centroid_func=centroid_function,
            )
            ...
            if show_individual_sources:
                for xcen, ycen in zip(x_centroids, y_centroids):
                    centroid = np.transpose(np.array([xcen, ycen]))
    ...
    def get_radial_bkg(radial_profile):
        ...
        boo = (
            radial_profile.profile < std
        )  # get the boolean list that picks up the radius larger than 1 sigma
        if sum(boo) == 0:
            bkg = None
        else:
    ...
    def get_mad_clipped_average(data, mad_sigma=3):
        ...
        mad = median_absolute_deviation(
            data.flatten()
        )  # this can be zero, which fails the calculation of boolean1 and boolean2
    ...
    def find_fwhm(
        ...
        centroid_dictionary = self.find_centroids(
            std_clip_threshold=std_clip_threshold,
            local_peak_detection_threshold=local_peak_detection_threshold,
            fit_box_size=fit_box_size,
            centroid_function=centroid_com,
        ...
        for image_path, centroids in zip(
            centroid_dictionary["image_path"], centroid_dictionary["centroids"]
        ):
            ...
            for centroid in centroids:
                ...
                fwhm = SourceDetection.find_fwhm_from_image(
                    image_path,
                    centroid,
                    edge_radii_end=edge_radii_end,
                    edge_radii_step=edge_radii_step,
                )
                fwhm_all += [fwhm]

        fwhm_all = np.array(fwhm_all)
        fwhm_all = fwhm_all[fwhm_all < 99]  # remove the failed FWHM fits
        averaged_fwhm, median_fwhm, stddev_fwhm = sigma_clipped_stats(
            fwhm_all, sigma=3, maxiters=10
        )  #
        print("The FWHMs of the image collection are:")
        print(f"{fwhm_all}")
