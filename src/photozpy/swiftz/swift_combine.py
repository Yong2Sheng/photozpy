...
class SwiftCombine():
    ...
    def __init__(self, image_collection, telescope):
        ...
        if isinstance(image_collection, ImageFileCollection):
            self._mcollection = mImageFileCollection(
                ...
                filenames=image_collection.files)  # this might contain unwanted fits files
    ...
    def sum_extensions(self, fits_file_path, out_path=None,
                       delete_files=False, return_full_path=True):
        """
        ...
        if not SwiftCombine.check_ASPCORR(fits_file_path):
            ...
            print(
                f"Skipping {fits_file_name} since the ASPECT isn't corrected properly! It will be deleted.")
            return
            ...
            if out_path is None:
                # if the out_name is not given, the file will be saved as the
                # filter name.
                out_path = fits_file_path.parent / f"{filter_}.fits"

            print(
                f"uvotimsum infile={fits_file_path} outfile={out_path} clobber=yes cleanup=yes | tee -a uvotimsum_log.txt >/dev/null 2>&1")
            os.system(
                f"uvotimsum infile={fits_file_path} outfile={out_path} clobber=yes cleanup=yes | tee -a uvotimsum_log.txt >/dev/null 2>&1")
            ...
            print(
                f"Extensions in {fits_file_name} has been summed to {out_path_name}\n")
    ...
    def sum_fits_files(self, image_collection,
        ...
        collection_filter = mImageFileCollection._get_header_values(
            image_collection,
            "FILTER",
            unique=True)  # collection_filter is list even if there is only one value
        ...
        if collection_filter not in self._telescope.filters:
        ...
        if len(fits_file_path) > 0:
            ...
            appended_file = Path(fits_file_path[0]).with_name(
                f"appended_{collection_filter}.fits")
    ...
    def sum_all_files(self, delete_files=False):
        ...
        for collection in self._mcollection:
            ...
            # ('data', 'B3_0850+443')[-1] is 'B3_0850+443'
            target_name = target_dir.parts[-1]
            ...
            for filter_ in self._telescope.filters:
                ...
                if len(files) == 0:
                    ...
                    summed_path = self.sum_extensions(
                        fits_file_path=files[0], return_full_path=True)
                    ...
                    for i in np.arange(num):
                        ...
                        if _summed is not None:
                            with fits.open(_out_path, mode="update") as hdul:
                                ...
                                hdul.flush()
                            # collect the file path of summed observations
                            summed_observations += [_summed]

                    # sum the observations
                    _collection = ImageFileCollection(
                        location=target_dir, filenames=summed_observations)
                    summed_path = self.sum_fits_files(
                        image_collection=_collection, delete_files=delete_files)
                    if summed_path is not None:
                        with fits.open(summed_path, mode="update") as hdul:
                            hdul[0].header["OBJECT"] = target_name.replace(
                                "_", " ")
