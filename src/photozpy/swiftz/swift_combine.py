...
class SwiftCombine:
    ...
    def __init__(self, image_collection, telescope):
        ...
        if isinstance(image_collection, ImageFileCollection):
            ...
            )  # this might contain unwanted fits files
    ...
    def sum_extensions(
        self, fits_file_path, out_path=None, delete_files=False, return_full_path=True
    ):
        """
        ...
        if not SwiftCombine.check_ASPCORR(fits_file_path):
            ...
            if out_path is None:
                out_path = (
                    fits_file_path.parent / f"{filter_}.fits"
                )  # if the out_name is not given, the file will be saved as the filter name.
