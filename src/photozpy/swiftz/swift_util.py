...
import pandas as pd
import numpy as np
from ..convenience_functions import del_then_create_folder, ungz_file
...
class SwiftDownload:
    ...
    def _standardize_obsquery_input(self, source_dict):
        ...
        if "obsid" in source_dict.keys():
            ...
            ra_dec = {
                key: value for key, value in source_dict.items() if key in {"ra", "dec"}
            }
    ...
    def set_obsquery_info_from_csv(self, csv_file):
        ...
        self.master_info = pd.read_csv(
            csv_file, sep=","
        )  # this is the DataFrame that contains all the information. New information will be added during the analysis pipeline.
        source_dict = self.master_info.to_dict(orient="list")

        self.obsquery_info = self._standardize_obsquery_input(
            source_dict
        )  # this is the dictionary usd to query and download data
    ...
    @staticmethod
    def slice_dict(
        input_dict, slice_index=None, slice_value=None, slice_key=None, remove_keys=None
    ):
        """
        ...
        for key, value in input_dict.items():
            if not isinstance(value, list):
                raise ValueError(
                    "The value of each key must be a list or SkyCoord to be sliced by index."
                )
            else:
    ...
    def download_swift_data(self, radius=5 / 60,
        ...
        for i in np.arange(target_num):
            target_info = SwiftDownload.slice_dict(
                input_dict=self.obsquery_info,
                slice_index=int(i),
                remove_keys=["window_lower", "window_upper"],
            )
            target_name = target_info["name"][
                0
            ]  # use [0] here because the values for each key is always a list event if there is only one element.

            oq = ObsQuery(
                radius=radius,
                begin=self.obsquery_info["window_lower"][i],
                end=self.obsquery_info["window_upper"][i],
                **target_info,
            )
            ...
            target_dir = del_then_create_folder(
                folder_dir=self.download_dir / target_name.replace(" ", "_")
    ...
    def organize_files(self):
        """
        Organize the download files. The file that are already organized will be overwritten.
        """

        source_names = self.obsquery_info["name"]
        for source_name in source_names:
            src_dir = self.download_dir / source_name.replace(
                " ", "_"
            )  # linux doesn't work with spaces in path
            files = list(src_dir.rglob("*sk*.img.gz"))
            for file in files:
                fits_path = ungz_file(
                    file_path=file, out_dir=src_dir, return_path=True, overwrite=True
                )
