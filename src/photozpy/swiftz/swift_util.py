...
class SwiftDownload():
    ...
    def _standardize_obsquery_input(self, source_dict):
        ...
        if "obsid" in source_dict.keys():
            ...
            ra_dec = {key: value for key, value in source_dict.items() if key in {
                "ra", "dec"}}
    ...
    def set_obsquery_info_from_csv(self, csv_file):
        ...
        # this is the DataFrame that contains all the information. New
        # information will be added during the analysis pipeline.
        self.master_info = pd.read_csv(csv_file, sep=",")
    ...
    @staticmethod
    def slice_dict(input_dict, slice_index=None, slice_value=None,
                   slice_key=None, remove_keys=None):
        """
    ...
    def download_swift_data(self, radius=5 / 60,
        ...
        for i in np.arange(target_num):
            target_info = SwiftDownload.slice_dict(
                input_dict=self.obsquery_info, slice_index=int(i), remove_keys=[
                    "window_lower", "window_upper"])
            # use [0] here because the values for each key is always a list
            # event if there is only one element.
            ...
            target_dir = del_then_create_folder(
                folder_dir=self.download_dir /
                target_name.replace(
                    " ",
                    "_"))  # linux doesn't work with spaces in path
