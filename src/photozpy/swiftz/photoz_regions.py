...
class PhotozRegions:
        ...
        for collection in self._mcollection:
            ...
            if bkg_path.exists():
                ...
                ra = self._source_catalog_df[
                    self._source_catalog_df["name"] == source_name
                ]["ra"].to_numpy()[0]
                dec = self._source_catalog_df[
                    self._source_catalog_df["name"] == source_name
                ]["dec"].to_numpy()[0]
    ...
    def get_source_regions(
        ...
        save_image=True,
        verbose=True,
        bkg_region_template=False,
        centroid_method=centroid_quadratic,
        aladin=True,
    ):
        """
        ...
        for collection in self._mcollection:
            ...
            for image_path in collection.files_filtered(
                ...
                if np.isnan(
                    x_centroids.astype(float)
                ).any():  # check is any of the fitted pixel coordinate is NaN
                ...
                centroid_skycoord = convert_coords(
                    wcs=ccddata.wcs,
                    skycoords=None,
                    ...
                    0
                ]  # the converted Skycoord is a list, so event if there is only one set skycoord, I have to use [0] to convert it from a list of skycoord to a skycoord

                src_region_path = PhotozRegions.generate_regions(
                    region_dir=image_path.parent,
                ...
                if save_image:
                    PhotozRegions._plot_regions(
                        ...
                        other_coords=[sky_coord],
                        other_coord_labels=["original source location"],
            ...
            if aladin is True:
                ...
                sky_region_ = CircleSkyRegion(
                    sky_coord, radius=5 * u.arcsec
                )  # the original sky region defined by the source catalog
                get_alain_image(
                    wcs=ccddata.wcs,
                    save_dir=image_path.parent,
                    sky_region=sky_region_,
                    min_cut=0.5,
                    max_cut=99.5,
                    logstretch=10,
                    source_name=source_name,
    ...
    def _plot_regions(
        ...
        filter_name=None,
        source_name=None,
        src_region_path=None,
        bkg_region_path=None,
        other_coords=None,
        other_coord_labels=None,
        image_cutout=["full_image", 100, 20, 120],
        log_stretch=3000,
        save_dir=None,
        save_image=False,
    ):
        ...
        axs[0, 1].set_xlim(
            source_pixelregion.center.x - image_cutout[1],
            source_pixelregion.center.x + image_cutout[1],
        )
        axs[0, 1].set_ylim(
            source_pixelregion.center.y - image_cutout[1],
            source_pixelregion.center.y + image_cutout[1],
        )
        # axs[0,1].legend()
        axs[0, 1].set_title(
            "Source (and background) region)", fontsize=subtitle_fontsize
        )
        ...
        axs[1, 0].set_xlim(
            source_pixelregion.center.x - image_cutout[2],
            source_pixelregion.center.x + image_cutout[2],
        )
        axs[1, 0].set_ylim(
        ...
        axs[1, 1].set_xlim(
            bkg_pixelregion.center.x - image_cutout[3],
            bkg_pixelregion.center.x + image_cutout[3],
        )
        axs[1, 1].set_ylim(
        ...
        fig.savefig(
            save_dir / f"{source_name}_{filter_name}.png", dpi=300, bbox_inches="tight"
        )
        ...
        for image_collection in self._mcollection:
            for source_name, source_coords in zip(
                ...
                for image_path in image_paths:
                    ...
                    source_coords_centroids = get_centroids(
                        sky_coords=source_coords,
                        array_data=image_array_data,
                        image_wcs=image_wcs,
                        return_type="sky_coord",
                    ...
                    src_region_path = generate_region_files(
                        ...
                        filter_name=image_filter_name,
                        sky_coord=source_coords_centroids,
                        region_type="src",
                        pixel_scale=pixel_scale,
                        inner_radius=aperture_radius,
                    )

                    bkg_region_path = generate_region_files(
                        region_save_dir=image_parent_path,
                        field_name=source_name,
                        source_name=target_names,
                        filter_name=image_filter_name,
                    ...
                    if save_plots is True:
                        plot_regions(
                            ...
                            image_wcs=image_wcs,
                            image_filter_name=image_filter_name,
                            field_name=source_name,
                            src_region_path=src_region_path,
                            bkg_region_path=bkg_region_path,
                            aladin_image=None,
                            additional_coords=source_coords,
                            additional_coords_labels="raw coordinate",
                            aladin_stretch=3000,
                            data_strentch=5000,
        ...
        for _ra, _dec, _name in zip(ra, dec, source_name):
            ...
            if inner_radius.unit.name == "pix":
                inner_radius_angular = inner_radius.to(
                    u.arcsec, pixel_scale
                ).value  # convert radius from pixel to angular units
...
def get_centroids(
    ...
    if sky_coords is not None:
        ...
        pixel_coords = convert_coords(
            wcs=image_wcs, skycoords=sky_coords, pixelcoords=None, verbose=False
        )
    ...
    while iters < niters:
        ...
        x_centroids_, y_centroids_ = centroid_sources(
            array_data_no_bkg,
            pixel_coords_[:, 0],
            pixel_coords_[:, 1],
            box_size=box_size,
    ...
    if idx_refit.size != 0:  # refit the failed coordinates only
        ...
        x_centroids_refit, y_centroids_refit = centroid_sources(
            array_data_no_bkg,
            pixel_coords_refit[:, 0],
            pixel_coords_refit[:, 1],
            box_size=box_size,
            centroid_func=centroid_com,
        )
...
def plot_regions(
    image_path=None,
    hdu=1,
    image_array_data=None,
    image_wcs=None,
    image_filter_name=None,
    field_name=None,
    src_region_path=None,
    bkg_region_path=None,
    aladin_image=None,
    additional_coords=None,
    additional_coords_labels=None,
    image_cutout=["full_image", 100, 20, 120],
    aladin_stretch=3000,
    data_strentch=3000,
    save_dir=None,
    ...
    fig, axs = plt.subplots(
        n_subplots,
        2,
        figsize=(15, 7.5 * n_subplots),
        subplot_kw=dict(projection=image_wcs),
    ...
    if additional_coords is not None:
        ...
        if additional_coords_labels is None:
            ...
            additional_coords_labels = [additional_coords_labels] * len(
                pix_region_additional
            )
    ...
    if (
        bkg_pixel_regions[0] is not None
    ):  # use bkg region to show and label the sources as the first option since it has larger radius and easier to see
        for idx, bkg_region in enumerate(bkg_pixel_regions):
            bkg_region.plot(
                ax=axs[0, 0],
                lw=1.0,
                label=bkg_region.meta["text"],
    ...
    for src_region, bkg_region, idx, idx_regs in zip(
        src_pixel_regions,
        bkg_pixel_regions,
        np.arange(1, n_subplots),
        ...
        if src_region is not None:
            ...
            axs[idx, 0].set_xlim(
                src_region.center.x - image_array_data.shape[0] * 0.05,
                src_region.center.x + image_array_data.shape[0] * 0.05,
            )
            axs[idx, 0].set_ylim(
                src_region.center.y - image_array_data.shape[1] * 0.05,
                src_region.center.y + image_array_data.shape[1] * 0.05,
            )
            axs[idx, 0].set_title(
                f"Source Region for {src_region.meta['text']}",
                fontsize=subtitle_fontsize,
            )
            if plot_additional_coords is True:
                axs[idx, 0].scatter(
                    pix_region_additional[idx_regs].center.x,
                    pix_region_additional[idx_regs].center.y,
                    marker="+",
                    s=marker_size,
                    color="orange",
                    label=additional_coords_labels[idx_regs],
                )
        ...
        if bkg_region is not None:
            ...
            axs[idx, 1].set_xlim(
                bkg_region.center.x - image_array_data.shape[0] * 0.05,
                bkg_region.center.x + image_array_data.shape[0] * 0.05,
            )
            axs[idx, 1].set_ylim(
                bkg_region.center.y - image_array_data.shape[1] * 0.05,
                bkg_region.center.y + image_array_data.shape[1] * 0.05,
            )
            axs[idx, 1].set_title(
                f"Background Region for {bkg_region.meta['text']}",
                fontsize=subtitle_fontsize,
            )
            if plot_additional_coords is True:
                axs[idx, 1].scatter(
    ...
    if save_image is True:
        fig.savefig(
            save_dir / f"{field_name}_{image_filter_name}_regions.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
