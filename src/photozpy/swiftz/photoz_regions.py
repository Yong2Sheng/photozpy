...
class PhotozRegions():
    ...
    def get_initial_bkg_regions(self):
        ...
        for collection in self._mcollection:
            ...
            if bkg_path.exists():
                ...
                ra = self._source_catalog_df[self._source_catalog_df["name"] == source_name]["ra"].to_numpy()[
                    0]
                dec = self._source_catalog_df[self._source_catalog_df["name"] == source_name]["dec"].to_numpy()[
                    0]
                sky_coord = SkyCoord(ra=ra, dec=dec, unit="deg", frame="fk5")
                print(f"ra = {ra}, dec = {dec}")

                PhotozRegions.generate_regions(
                    region_dir=collection_path,
                    filter_name="bkg",
                    coord=sky_coord,
                    radius=30)
    ...
    def get_source_regions(self, box_size=11, save_image=True, verbose=True,
                           bkg_region_template=False, centroid_method=centroid_quadratic, aladin=True):
        """
        ...
        for collection in self._mcollection:
            ...
            for image_path in collection.files_filtered(
                ...
                if np.isnan(x_centroids.astype(float)).any(
                ):  # check is any of the fitted pixel coordinate is NaN

                    print(
                        f"The fitting for {filter_name} using {centroid_method.__name__} failed, switch to {centroid_com.__name__} instead!")
                ...
                if bkg_region_template is not None:
                    ...
                    bkg_template_path = image_path.parent / bkg_region_template
                ...
                if save_image:
                    PhotozRegions._plot_regions(array_data=array_data, wcs=ccddata.wcs, filter_name=filter_name, source_name=source_name,
                                                ...
                                                other_coords=[sky_coord], other_coord_labels=[
                                                    "original source location"],
                                                save_dir=image_path.parent, save_image=True)

            if aladin is True:
                print(f"Querying aladin image for {source_name}")
                # the original sky region defined by the source catalog
                sky_region_ = CircleSkyRegion(sky_coord, radius=5 * u.arcsec)
    ...
    def _plot_regions(image_path=None, hdu=1, array_data=None, wcs=None, filter_name=None, source_name=None,
        ...
        if src_region_path is not None:
            ...
            axs[0,
                0].scatter(source_pixelregion.center.x,
                           source_pixelregion.center.y,
                           marker="+",
                           s=10,
                           color="lime",
                           label="source region center")

            # source (and background), upper right
            axs[0,
                1].scatter(source_pixelregion.center.x,
                           source_pixelregion.center.y,
                           marker="+",
                           s=10,
                           color="lime",
                           label="source region center")
            source_pixelregion.plot(
                ax=axs[0, 1], color='lime', lw=1.0, label="source region")
            ...
            axs[1,
                0].scatter(source_pixelregion.center.x,
                           ...
                           label="source region center")
            source_pixelregion.plot(
                ax=axs[1, 0], color='lime', lw=1.0, label="source region")

        if bkg_region_path is not None:
            # full image, upper left
            bkg_pixelregion.plot(
                ax=axs[0, 0], color='red', lw=1.0, label="background region")

            # source (and background), upper right
            bkg_pixelregion.plot(
                ax=axs[0, 1], color='red', lw=1.0, label="background region")

            # background, lower right
            bkg_pixelregion.plot(
                ax=axs[1, 1], color='red', lw=1.0, label="background region")

        # plot images to subplots
        # the full image
        axs[0, 0].imshow(array_data, origin='lower', norm=norm,
                         cmap='Greys_r', interpolation='nearest')
        # axs[0,0].legend()
        axs[0, 0].set_title("Full sky image", fontsize=subtitle_fontsize)

        # source (and background), upper right
        axs[0, 1].imshow(array_data, origin='lower', norm=norm,
                         cmap='Greys_r', interpolation='nearest')
        axs[0, 1].set_xlim(source_pixelregion.center.x -
                           image_cutout[1], source_pixelregion.center.x +
                           image_cutout[1])
        axs[0, 1].set_ylim(source_pixelregion.center.y -
                           image_cutout[1], source_pixelregion.center.y +
                           image_cutout[1])
        # axs[0,1].legend()
        axs[0, 1].set_title('Source (and background) region)',
                            fontsize=subtitle_fontsize)

        # the source regions zoomed in, side wdith = 100
        axs[1, 0].imshow(array_data, origin='lower', norm=norm,
                         cmap='Greys_r', interpolation='nearest')
        axs[1, 0].set_xlim(source_pixelregion.center.x -
                           image_cutout[2], source_pixelregion.center.x +
                           image_cutout[2])
        axs[1, 0].set_ylim(source_pixelregion.center.y -
                           image_cutout[2], source_pixelregion.center.y +
                           image_cutout[2])
        # axs[1,0].legend()
        ...
        axs[1, 1].imshow(array_data, origin='lower', norm=norm,
                         cmap='Greys_r', interpolation='nearest')
        axs[1, 1].set_xlim(bkg_pixelregion.center.x -
                           image_cutout[3], bkg_pixelregion.center.x +
                           image_cutout[3])
        axs[1, 1].set_ylim(bkg_pixelregion.center.y -
                           image_cutout[3], bkg_pixelregion.center.y +
                           image_cutout[3])
        # axs[1,1].legend()
        ...
        for ax in axs.flatten():
            ...
            if other_coords is not None:
                for coord_, label_ in zip(other_coords, other_coord_labels):
                    other_pixel_coord = convert_coords(
                        wcs=wcs, skycoords=coord_, pixelcoords=None, verbose=False)
                    ax.scatter(
                        other_pixel_coord[0][0],
                        other_pixel_coord[0][1],
                        marker="+",
                        s=10,
                        color="orange",
                        label=label_)
        ...
        fig.savefig(
            save_dir /
            f"{source_name}_{filter_name}.png",
            dpi=300,
            bbox_inches="tight")
...
class CCD_Regions():
    ...
    def __init__(self, image_collection, sources):
        ...
        if isinstance(image_collection, ImageFileCollection):
            self._mcollection = mImageFileCollection(
                image_dir=image_collection.location,
                filenames=image_collection.files)
    ...
    def generate_ccd_regions(self, save_plots=True, hdu=0, box_size=51 *
                             u.pixel, aperture_scaling=3, save_location=None, **kwargs):
        """
        ...
        **kwargs
        ...
        for image_collection in self._mcollection:
            for source_name, source_coords in zip(
                    self._sources.source_names, self._sources.coords):
                image_paths = image_collection.files_filtered(
                    include_path=True, **{"OBJECT": source_name})
                ...
                for image_path in image_paths:
                    ...
                    # get the centroids of the sky coordinates
                    ...
                    print(
                        f"{source_name} in {image_filter_name} has centroid coordinates {source_coords_centroids}")

                    # get the pixel scale
                    # x and y are pixel scale along two axis. They are usually
                    # very close.
                    x, y = wcs.utils.proj_plane_pixel_scales(
                        image_wcs) * u.deg / u.pixel
                    # I choose x axis as the pixel scale for conversion.
                    pixel_scale = u.pixel_scale(x)

                    # now the target names are generated automatically
                    target_names = [
                        f"S{i}" for i in np.arange(
                            source_coords_centroids.shape[0])]
                    ...
                    if save_plots is True:
                        plot_regions(image_array_data=image_array_data, image_wcs=image_wcs, image_filter_name=image_filter_name, field_name=source_name,
                                     ...
                                     additional_coords=source_coords, additional_coords_labels="raw coordinate",
...
def generate_region_files(region_save_dir, field_name, source_name, filter_name,
    ...
    if len(source_name) != len(ra):
        raise ValueError(
            "The number of ra and dec doesn't match the number of source names.")
    ...
    with open(region_path, "w") as f:
        ...
        for _ra, _dec, _name in zip(ra, dec, source_name):
            ...
            if inner_radius.unit.name == "pix":
                # convert radius from pixel to angular units
                inner_radius_angular = inner_radius.to(
                    u.arcsec, pixel_scale).value
            if outer_radius is None:
                f.write(
                    f'circle({_ra},{_dec},{inner_radius_angular}") # text={{{_name}_{region_type}}}\n')
            else:
...
def get_centroids(sky_coords=None, pixel_coords=None, image_path=None, hdu=None, array_data=None, image_wcs=None, filter_name=None,
    ...
    if sky_coords is not None:
        # convert to the pixel coordinate
        pixel_coords = convert_coords(
            wcs=image_wcs,
            skycoords=sky_coords,
            pixelcoords=None,
            verbose=False)
    ...
    if idx_refit.size != 0:  # refit the failed coordinates only
        ...
        print(
            f"The centroid fit FAILED for the following coordinate using {centroid_method.__name__}: {sky_coords_refit}!")
        print(f"Switch to {centroid_com.__name__}.")

        x_centroids_refit, y_centroids_refit = centroid_sources(array_data_no_bkg,
                                                                pixel_coords_refit[:,
                                                                                   0], pixel_coords_refit[:, 1],
                                                                box_size=box_size,
                                                                centroid_func=centroid_com)
...
def plot_regions(image_path=None, hdu=1, image_array_data=None, image_wcs=None, image_filter_name=None, field_name=None,
    ...
    additional_coords : astropy.coordinates.SkyCoord
        The additional sky coordinates to plot. They will be plotted as scattered crosses.
    ...
    if n_src_regions is None and n_bkg_regions is None:
        if n_src_regions != n_bkg_regions:
            raise ValueError(
                f"The number of source and background are not equal: source regions: {n_src_regions}; background regions: {n_bkg_regions}.")
        else:
    ...
    # Create figure
    fig, axs = plt.subplots(n_subplots, 2, figsize=(
        15, 7.5 * n_subplots), subplot_kw=dict(projection=image_wcs), constrained_layout=True)
    fig.suptitle(
        f"Region summary for {image_filter_name} filter of source {field_name}",
        fontsize=20)
    text_fontsize = 15
    tick_fontsize = 15
    ...
    # prepare the additional coordinates and labels
    if additional_coords is not None:
        plot_additional_coords = True
        # Since I just want to plot the centers, the radius 5 arcsec is my
        # random choice
        sky_region_additional = [CircleSkyRegion(
            i, 5 * u.arcsec) for i in additional_coords]
        pix_region_additional = [
            i.to_pixel(image_wcs) for i in sky_region_additional]

        if additional_coords_labels is None:
            additional_coords_labels = [
                "additional coordinates"] * len(pix_region_additional)
        elif isinstance(additional_coords_labels, str):
            additional_coords_labels = [
                additional_coords_labels] * len(pix_region_additional)
        elif isinstance(additional_coords_labels, list):
            if len(additional_coords_labels) != len(pix_region_additional):
                raise ValueError(
                    "The number of additional coordinates is not equal to the number of labels for the additional coordinates!")
        else:
            raise ValueError(
                "The accepted labels for the additional coordinates are string or list!")
    else:
    ...
    # if bkg_pixel_regions[0] is not None:  # use bkg region to show and label the sources as the first option since it has larger radius and easier to see
    ...
    axs[0, 1].imshow(image_array_data, origin='lower', norm=norm,
                     cmap='Greys_r', interpolation='nearest')
    axs[0, 1].set_title("Data Image", fontsize=subtitle_fontsize)
    # use bkg region to show and label the sources as the first option since
    # it has larger radius and easier to see
    if bkg_pixel_regions[0] is not None:
        for bkg_region in bkg_pixel_regions:
            bkg_region.plot(
                ax=axs[0, 1], lw=1.0, label=bkg_region.meta["text"], color=np.random.rand(3,))
            axs[0,
                1].text(bkg_region.center.x,
                        bkg_region.center.y,
                        bkg_region.meta["text"],
                        color="white",
                        size=text_fontsize)
        ...
        for src_region in src_pixel_regions:  # src region is the second option
            src_region.plot(
                ax=axs[0, 1], lw=1.0, label=src_region.meta["text"], color=np.random.rand(3,))
            axs[0,
                1].text(src_region.center.x,
                        src_region.center.y,
                        src_region.meta["text"],
                        color="white",
                        size=text_fontsize)

    # start plotting the regions
    for src_region, bkg_region, idx, idx_regs in zip(src_pixel_regions, bkg_pixel_regions, np.arange(
            1, n_subplots), np.arange(0, len(src_pixel_regions))):
        ...
        if src_region is not None:
            axs[idx, 0].imshow(image_array_data, origin='lower',
                               norm=norm, cmap='Greys_r', interpolation='nearest')
            axs[idx,
                0].scatter(src_region.center.x,
                           src_region.center.y,
                           marker="+",
                           s=marker_size,
                           color="lime",
                           label=f"{src_region.meta['text']} source region center")
            src_region.plot(ax=axs[idx, 0], color='lime', lw=1.0,
                            label=f"{src_region.meta['text']} source region")
            axs[idx, 0].set_xlim(src_region.center.x -
                                 image_array_data.shape[0] *
                                 0.05, src_region.center.x +
                                 image_array_data.shape[0] *
                                 0.05)
            axs[idx, 0].set_ylim(src_region.center.y -
                                 image_array_data.shape[1] *
                                 0.05, src_region.center.y +
                                 image_array_data.shape[1] *
                                 0.05)
            axs[idx, 0].set_title(
                f"Source Region for {src_region.meta['text']}", fontsize=subtitle_fontsize)
        ...
        if bkg_region is not None:
            axs[idx, 1].imshow(image_array_data, origin='lower',
                               norm=norm, cmap='Greys_r', interpolation='nearest')
            axs[idx,
                1].scatter(bkg_region.center.x,
                           bkg_region.center.y,
                           marker="+",
                           s=marker_size,
                           color="red",
                           label=f"{bkg_region.meta['text']} background center")
            bkg_region.plot(ax=axs[idx, 1], color='red', lw=1.0,
                            label=f"{bkg_region.meta['text']} background annulus")
            axs[idx, 1].set_xlim(bkg_region.center.x -
                                 image_array_data.shape[0] *
                                 0.05, bkg_region.center.x +
                                 image_array_data.shape[0] *
                                 0.05)
            axs[idx, 1].set_ylim(bkg_region.center.y -
                                 image_array_data.shape[1] *
                                 0.05, bkg_region.center.y +
                                 image_array_data.shape[1] *
                                 0.05)
            axs[idx, 1].set_title(
                f"Background Region for {bkg_region.meta['text']}", fontsize=subtitle_fontsize)
            if plot_additional_coords is True:
    ...
    if save_image is True:
        fig.savefig(
            save_dir /
            f"{field_name}_{image_filter_name}_regions.png",
            dpi=150,
            bbox_inches="tight")
        plt.close()
